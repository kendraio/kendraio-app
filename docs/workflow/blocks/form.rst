...
^^^^^^^^^^^^^^^^^^^^
The form accepts a `schemaGetter` property, which can be used to fetch a schema to generate the form.
...
This Flow shows an example of how to use it with a string: `<https://app.kendra.io/kendraio/schema-form>`_ 
...
A mapping block can be used to grab the schema from data.
