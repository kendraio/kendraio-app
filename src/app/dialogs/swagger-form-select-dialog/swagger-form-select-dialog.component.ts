...
export class SwaggerFormSelectDialogComponent implements OnInit {
  ...
  ngOnInit() {
    this.adapters$ = this.adapters.adapters$.pipe(
      ...
      map((adapters) => {
        return Object.keys(adapters).map((adapterName) => ({
          adapterName,
          swagger: adapters[adapterName]['adapter'].swagger,
        }));
      }),
      switchMap((adapters) => {
        return forkJoin(
          adapters.map(({ adapterName, swagger }) =>
            this.http
              .get(`${environment.adapterBaseUrl}${swagger}`)
              .pipe(map((result) => ({ adapterName, result })))
          )
        );
      }),
      map((adapters) =>
        adapters.reduce((a, { adapterName, result }) => {
