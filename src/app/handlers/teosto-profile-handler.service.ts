...
export class TeostoProfileHandlerService {
  constructor(private readonly formSubmit: FormSubmitHandlerService) {
    this.formSubmit.actions$
      .pipe(filter(({ form }) => form === "teosto:profile"))
      .subscribe(({ payload }) => {
  ...
  getProfile() {
    const teostoProfile = JSON.parse(
      localStorage.getItem("teosto-user-profile"),
    );
