...
exports.Converter = exports.convertTemplateToSchema = void 0;
...
function convertTemplateToSchema(template, config) {
  var converter = new Converter(template, config);
  return converter.convert();
}
exports.convertTemplateToSchema = convertTemplateToSchema;
/**
 * The converter class.
 *
 * @class Converter
...
var Converter = /** @class */ (function () {
   ...
   * @memberof Converter
   */
  function Converter(template, config) {
    this.blockTypeDefaults = (0, lodash_es_1.get)(
      config,
      'blockTypeDefaults',
      {}
    );
    this.convertables = (0, lodash_es_1.get)(config, 'convertables', {});
  ...
  Converter.prototype.convert = function () {
    ...
    return this.convertToValidJSONSchema(
      this.arrayOfUUIDsToDict(
        this.applyBlockTypeDefaults(this.convertKeys(this.template))
      )
    );
  };
  /**
   ...
   * @memberof Converter
   */
  Converter.prototype.uuid = function () {
    return (0, uuid_1.v4)();
  };
  /**
   ...
   * @memberof Converter
   */
  Converter.prototype.convertKeys = function (object) {
    var newObject = (0, lodash_es_1.clone)(object);
    for (var key in object) {
      if (this.convertables[key]) {
        ...
        if (typeof object[key] === 'object') {
          // Recurse into objects, but not arrays.  Arrays are handled below. We could also check the type of the value at each level and handle it accordingly.
   ...
   * @memberof Converter
   */
  Converter.prototype.applyBlockTypeDefaults = function (object) {
    // We recursively loop through the object and apply blockTypeDefaults to each block-content key.
    // This means that when we find an object with a UUID matching an expected key, we replace it's contents entirely with a copy of the defaults.
    var targetKeyName = 'block-content'; // TODO: depreciate this hardcoded CMS specific property.
    var newObject = (0, lodash_es_1.clone)(object); // Make a copy of the original object.
    ...
    for (var key in object) {
      ...
      if (typeof object[key] === 'object') {
        // If it is an array or an object, recurse into it:
        ...
        if (Array.isArray(newObject[key]) && newObject[targetKeyName]) {
          // If we have a block-content array that contains objects with uuids matching those of our defaults...
          var tempArray = [];
          for (var i = 0; i < newObject[key].length; i++) {
            // Loop through the array of objects.
            if (blockTypeDefaultKeys.indexOf(newObject[key][i]['uuid']) > -1) {
              // If we find a UUID that matches one of our defaults...
              tempArray.push(
                (0, lodash_es_1.clone)(
                  this.blockTypeDefaults[newObject[key][i]['uuid']]
                )
              ); // Push a copy of the default object into our temporary array, replacing it's contents entirely with those from blockTypeDefaults.
   ...
   * @memberof Converter
   */
  Converter.prototype.arrayOfUUIDsToDict = function (object) {
    // Converts arrays containing UUIDs to dictionaries with UUIDs as keys:
    ...
    for (var key in newObject) {
      ...
      if (typeof newObject[key] === 'object') {
        ...
        if (
          ...
          arrayChildrenAllHaveUUIDs(newObject[key])
        ) {
          var tempObj = {};
          for (var i = 0; i < newObject[key].length; i++) {
            // Loop through the original child objects in the original parent object's key value:
   ...
   * @memberof Converter
   */
  Converter.prototype.convertToValidJSONSchema = function (template) {
    // Converts the template to a valid JSON Schema.
    var schema = {
      type: 'object',
      properties: {},
