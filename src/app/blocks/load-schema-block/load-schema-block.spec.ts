...
describe("LoadSchemaBlockComponent", () => {
  ...
  it('should load a "property" schema and list of "person" records from the database', async () => {
    ...
    spyOn(component, "loadSchemaFromDatabase").and.callFake(
      async (schemaName) => {
    ...
    const oneOfPeople = get(
      component.lastOutput,
      "jsonSchema.definitions.property.properties.owner.oneOf",
    );
  ...
  it('should load a "team" schema with a members list to be populated by a list of "person" records from the database', async () => {
    ...
    const members = get(
      component.lastOutput,
      "jsonSchema.definitions.team.properties.members.items.oneOf",
    );
