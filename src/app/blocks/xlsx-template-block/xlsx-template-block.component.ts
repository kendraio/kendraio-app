...
export class XlsxTemplateBlockComponent extends BaseBlockComponent {
  ...
  onData(data: any, firstChange: boolean) {
    ...
    if (!isArray(this.model)) {
      if (this.model) {
        this.hasError = true;
        this.errorMessage = 'The XLSX template task requires an array of rows as data. Non-row data was provided.';
      }
      return;
    }
    ...
    this.http.get(url, {responseType: 'arraybuffer'})
      ...
      .subscribe(template => {
        ...
        this.hasError = true;
        this.errorMessage = isString(error) ? error : error.message || 'Error writing to XLSX template';
      });
  }
