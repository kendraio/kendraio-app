...
function loadCurrencyRates(assetCode, fiat) {
  ...
  return fetch(
    ...
    .then((response) => {
      const matching_asset = response.filter((item) => {
...
export class WebMoneyComponent extends BaseBlockComponent {
  ...
  onConfigUpdate(config: any) {
    ...
    this.supportMissingTemplateConfig = {
      template: this.supportMissingMessage,
    };
  ...
  setupPaymentWatcher() {
    ...
    function monetizationprogressHandler(event) {
      ...
      const lastStreamFloatTotalAmount =
        lastStreamTotalAmount * Math.pow(10, -detail.assetScale);
      ...
      if (detail.assetCode + fiat in rates) {
        const fiatTotalAmount =
          floatTotalAmount * rates[detail.assetCode + fiat];
  ...
  onData(data: any, _firstChange: boolean) {
    ...
    if (
      !!this.model.paymentPointer &&
      this.model.paymentPointer.length > 0 &&
      this.enabled
    ) {
