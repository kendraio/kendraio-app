...
export class MappingBlockComponent implements OnInit, OnChanges {
  ...
  handleStateChange(): void {
    ...
    const stateKeys = this.mapping.match(
      /state\.(global|local)\.[a-zA-Z0-9_]+/g
    );
    ...
    if (watchedStateDiffers) {
      ...
      if (window['queuedStateReactionWorker'] === undefined) {
        window['queuedStateReactionWorkerFunction'] = () => {
          ...
          window['queuedStateReactionWorker'] = setTimeout(
            window['queuedStateReactionWorkerFunction'],
            window['stateDeferTime']
          );
        };
        window['queuedStateReactionWorker'] = setTimeout(
          window['queuedStateReactionWorkerFunction'],
      ...
      const stateChangesInLastSecond = this.stateChangeInitTimestamps.filter(
        (t) => t > lastSecond
      );
      ...
      if (tooFast || this.slowMode) {
        ...
        window['stateDeferTime'] = Math.min(
          Math.floor(window['stateDeferTime'] * 1.28),
          4000
        );

        if (tooFast) {
          // to track possible loops, add our mapping string to the stateLoopRisks array
          window['stateLoopRisks'].push({
        ...
        if (this.scheduledRuns < 1) {
          window['queuedStateReaction'].push(() => {
            ...
            console.log(
              'Running throttled state change for mapping',
              this.mapping,
              'after waiting',
              window['stateDeferTime'] / 1000,
              'seconds'
            );
            this.ngOnChanges({});
          ...
          console.log(
            this.scheduledRuns,
            'state changes have occured for',
            this.mapping,
            'but we are waiting to process them.'
          );
        }
        return;
      }
    ...
    if (isObject(mapping)) {
      ...
      if (keys.length === 1) {
        ...
        if (keys[0] === '[') {
          return (
            '[' +
            mapping[keys[0]].map((m) => this.parseMapping(m)).join(', ') +
            ']'
          );
        }
        if (keys[0] === '(') {
        ...
        if (keys[0] === '{') {
          ...
          return (
            `{ ` +
            keys2.map((k) => `${k}: ${this.parseMapping(obj[k])}`).join(', ') +
            ` }`
          );
        }
        return keys
          .map((k) => `${k}${this.parseMapping(mapping[k])}`)
          .join(', ');
      }
      return (
        `{ ` +
        keys.map((k) => `${k}: ${this.parseMapping(mapping[k])}`).join(', ') +
        ` }`
      );
    }
    return '';
