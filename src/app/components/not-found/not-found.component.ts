    ...
    combineLatest([this.route.url, this.refresh$])
      .pipe(
        ...
        map((v) =>
          isArray(v) && v.length === 0
            ? [{ path: "core" }, { path: "dashboard" }]
            : v,
        ),
        map((v) =>
          isArray(v) && v.length === 1 && v[0].path && v[0].path === "dashboard"
            ? [{ path: "core" }, { path: "dashboard" }]
            : v,
        ),
        filter((v) => isArray(v) && v.length >= 2),
        withLatestFrom(this.route.queryParams),
        switchMap(([[adapterName, workflowId], queryParams]: any) =>
          this.workflowRepo.getBlocks(adapterName.path, workflowId.path).pipe(
      ...
      .subscribe(({ blocks, title, context, tags, queryParams, fragment }) => {
        this.workflow.initWorkflow({
          ...
          blocks,
          tags,
          context: { ...context, queryParams, fragment },
        });
        this.isLoaded = true;
      });
  }
