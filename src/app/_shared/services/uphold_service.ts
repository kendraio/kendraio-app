...
export function getUpholdOauthLoginURL(): string {
  ...
  const loginURL = new URL(
    UPHOLD_SITE_ROOT + 'authorize/' + environment.uphold.clientID
  );
...
export async function finishAuth(): Promise<void> {
  ...
  if (result.access_token) {
    ...
    // being incompatible
    if (
      decoded_path_name !== window.location.pathname &&
      decoded_path_name !== '/workflow-builder'
    ) {
...
export class UpholdService {
  ...
  xrp_address_balances: {
    [key: string]: { balance: number; ILP_URL?: string };
  } = {};
  // Uphold token:
  token: string;
  ...
  public async get_xrp_cards_list(only_balance: boolean) {
    ...
    await Promise.all(
      xrp_cards.map(async (card) => {
        // we get the balance for this card:
        const address_data = await oauth_get(
          'v0/me/cards/' + card.id + '/addresses'
        );
        // we extract the Interledger address from the response using JMESPath
        // assuming it exists:
        const ILP_address = search(
          address_data,
          `[?type=='interledger'].formats[0].value|[0]`
        );
        // we get the card from the list of results, to get the balance:
        const matchingCard = users_cards.filter(
        ...
        parent_this.xrp_address_balances[card.id.replace(/-/g, '')] = {
          balance: cardBalance,
          ILP_URL: ILP_address,
        };
        // if only_balance is true, we emit the balance for the specified address:
        if (
          parent_this.balance_address === ILP_address &&
          only_balance === true
        ) {
